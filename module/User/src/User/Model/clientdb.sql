...
--
-- Table structure for table `attempts_status`
--

DROP TABLE IF EXISTS `attempts_status`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `attempts_status` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `link_code` varchar(20) NOT NULL,
  `stu_id` int(11) NOT NULL,
  `elapsed_time` int(11) NOT NULL,
  `no_of_attempts` int(1) DEFAULT NULL,
  `close_count` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
...
for each row
BEGIN
insert into activity(activity_performed,entity_created,created_what,created_by,created_when) values('created','category',New.name,New.created_by,now());
END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER insert_in_activity_on_category_update_delete after update on `category`
for each row
BEGIN
if(New.status = 1) then
insert into activity(activity_performed,entity_created,created_what,created_by,created_when) values('deleted','category',New.name,New.updated_by,now());
else
insert into activity(activity_performed,entity_created,created_what,created_by,created_when) values('updated','category',concat(Old.name,' to ',New.name),New.updated_by,now());
end if;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
INSERT INTO `custom_instructions` VALUES (1,'should be finished in single sitting only,will not allow you to go back and make changes,has mandatory to select answer');
...
CREATE TABLE `link_settings` (
  ...
  `custom_instructions_id` int(11) DEFAULT '1',
  ...
  `test_questions` varchar(300) DEFAULT NULL,
  PRIMARY KEY (`id`),
...
for each row
BEGIN
insert into activity(activity_performed,entity_created,created_what,created_by,created_when) values('created','test',New.name,New.created_by,now());
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER insert_in_activity_on_test_update_delete after update on `test`
for each row
BEGIN
if(New.status = 1) then
insert into activity(activity_performed,entity_created,created_what,created_by,created_when) values('deleted','test',New.name,New.updated_by,now());
else
insert into activity(activity_performed,entity_created,created_what,created_by,created_when) values('updated','test',concat(Old.name,' to ',New.name),New.updated_by,now());
end if;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2014-07-24 13:09:07
