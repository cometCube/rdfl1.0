...
class UserTable
{
	...
	public function __construct(TableGateway $tableGateway)  
	{
		...
		$dbAdapterConfig2 = array(
				'driver'         =>  'Pdo',
				'dsn'            =>  'mysql:dbname=clientdb;host=localhost',
				'username' => 'root', 
				'password' =>'root' , 
		);
		$this->dbAdapter12 = new Adapter($dbAdapterConfig);
		$this->dbAdapter2 = new Adapter($dbAdapterConfig2);
	}
	...
	public function saveUser(User $register)
	{
		...
		try {
			...
			$select = $sql->insert('user_profile')
			...
			$result = $query->execute()->getResource()->closeCursor();
	...
	{
		$rowset = $this->tableGateway->select(array('id'=>$id,'status'=>'0'));
		$row = $rowset->current();
	...
	{
		$rowset = $this->tableGateway->select(array($usernameType=>$username,'status'=>'1'));
		$row = $rowset->current();
...
public function getPasswordByEmail($email)
 ...
 for ($i = 0; $i < $length-30; $i++) {
      $string .= $characters[rand(0, $length - 1)];
		...
		$update = $sql->update();
		$update->table('users');
		$update->set(array('password' => md5($string)));
	...
	//echo $update->getSqlString();
		...
		public function isEmailexist($txtVal){
			...
			$rowset = $this->tableGateway->select(array('email'=>$txtVal));
		 	$row = $rowset->current();	
	...
	public function checkVal($txtVal){
		...
		$pos = strpos($txtVal,'@');
		if($pos === false){
			$rowset = $this->tableGateway->select(array('username'=>$txtVal));
		 	$row = $rowset->current();	
		} else {
			$rowset = $this->tableGateway->select(array('email'=>$txtVal));
		 	$row = $rowset->current();	
	...
	public function userProfile($userid){
		...
		$sql = new Sql ( $this->tableGateway->getAdapter() );
		$select = $sql->select ()
					  ->from ( array ('usersTabl' => 'users' ) )
					  ->columns ( array ('clientId' => 'client_id',
					  					 'email' => 'email',
										 'username' => 'username',
										 'regDate' => 'registration_date' ) )
					  ->join ( array ('userProfileTabl' => 'user_profile' ), 'usersTabl.client_id = userProfileTabl.user_id',
	...
	public function clientGeneralProfileUpdate($profileData){
		...
		$update = $sql->update();
		$update->table('users');
		$update->set(array('first_name' => $profileData['profileFirstName'],
				           'last_name' => $profileData['profileLastName']
		            ));
		$update->where(array('testcube_id' => $profileData['clientId']));
		//echo $update->getSqlString();
		$statement = $sql->prepareStatementForSqlObject($update);
		try {
			$result = $statement->execute();        // works fine
...
else
{
    $update->set(array('first_name' => $profileData['profileFirstName'],
        ...
        $update->where(array('user_id' => $profileData['clientId']));
        //echo $update->getSqlString();
	...
	public function updateProfilePassword($passwordData){
		...
		$update = $sql->update();
		$update->table('users');
		$update->set(array('password' => md5($passwordData['newPassword'])));
		$update->where(array('client_id' => $passwordData['clientId']));
		//echo $update->getSqlString();
