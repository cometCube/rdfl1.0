...
class StudentTable 
{
	...
	public function __construct()  
	{
		...
		if($quizSession->offsetExists('quiz')) {
	...
	public function saveStudent(Student $Student)
	{
		...
		public function loadAttemptTime($StuIdAndQuestionId)
		{
			...
			if(isset($StuIdAndQuestionId[2]))
			{
				unset($StuIdAndQuestionId[2]);
			}
		...
		public function addSelectedOption($arrData)
		{
			...
			$this->attempts_status_entry($arrData); //entry in attempts_status_entry
			if($val_1)
			{
				$return = 1;
			}
		...
		public function attempts_status_entry($arrData)
		{
			$attempt_flag=0;
			$flag=$this->loadAttemptTime($arrData);
			$return = 0;
			if($flag!=0)
			{
			
				if(is_array($arrData[3]))
				{
					$return = 0;
					$insertIntoattempts_statusQuery = "UPDATE attempts_status 
															set
															elapsed_time='$arrData[4]'
															WHERE link_code='$arrData[3]'
